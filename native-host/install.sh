#!/bin/sh
# Agentao in Chrome — macOS/Linux installer (double-click to run)
# Runs the native host executable with --install. The extension ID is
# fixed via manifest.json's key field, so no --extension-id is needed.
