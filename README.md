...
- **技能（Skills）**：可扩展的站点/任务专属指引系统，在 `native-host/skills/` 下放置 `SKILL.md` 即可加载，无需改代码。
...
3. 点击**加载已解压的扩展程序**，选择刚解压的扩展文件夹。
4. 复制该扩展的 **ID**（一串 32 位字符）。
...
```bash
# macOS / Linux
./agentao-chrome-host --install --extension-id <你的扩展ID>

# Windows
agentao-chrome-host.exe --install --extension-id <你的扩展ID>
```
...
pip install agentao
python scripts/install_native_host.py --extension-id <你的扩展ID>
```
...
│   ├── agentao_chrome_host.json.template  # NM 清单模板
│   ├── skills/                   # 可扩展技能目录
