...
5. The extension ID is fixed via the manifest.json key field — no need to copy it manually.
...
Output lands in `Releases/dist/agentao-chrome-host/`. The `--onefile` flag
